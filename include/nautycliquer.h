...
/*
 * We #define _Boolean instead of using a typedef because nauty.h uses it
...
BDM: In nauty's version we will use nauty's _Boolean (which is int anyway).
 */
...
extern _Boolean graph_weighted(graph_t *g);
...
extern _Boolean graph_write_dimacs_ascii(graph_t *g, char *comment,FILE *fp);
extern _Boolean graph_write_dimacs_ascii_file(graph_t *g,char *comment,
					     char *file);
extern _Boolean graph_write_dimacs_binary(graph_t *g, char *comment,FILE *fp);
extern _Boolean graph_write_dimacs_binary_file(graph_t *g, char *comment,
					      char *file);

extern void graph_print(graph_t *g);
extern _Boolean graph_test(graph_t *g, FILE *output);
...
extern _Boolean reorder_is_bijection(int *order,int n);


#define reorder_by_default reorder_by_greedy_coloring
extern int *reorder_by_greedy_coloring(graph_t *g, _Boolean weighted);
extern int *reorder_by_weighted_greedy_coloring(graph_t *g, _Boolean weighted);
extern int *reorder_by_unweighted_greedy_coloring(graph_t *g,_Boolean weighted);
extern int *reorder_by_degree(graph_t *g, _Boolean weighted);
extern int *reorder_by_random(graph_t *g, _Boolean weighted);
extern int *reorder_by_ident(graph_t *g, _Boolean weighted);
extern int *reorder_by_reverse(graph_t *g, _Boolean weighted);


typedef struct _clique_options clique_options;
struct _clique_options {
	int *(*reorder_function)(graph_t *, _Boolean);
	int *reorder_map;

	/* arguments:  level, n, max, user_time, system_time, opts */
	_Boolean (*time_function)(int,int,int,int,double,double,
				 clique_options *);
	FILE *output;

	_Boolean (*user_function)(set_t,graph_t *,clique_options *);
	void *user_data;
	set_t *clique_list;
...
extern set_t clique_find_single(graph_t *g,int min_weight,int max_weight,
				_Boolean maximal, clique_options *opts);
extern int clique_find_all(graph_t *g, int req_weight, _Boolean exact,
			   _Boolean maximal, clique_options *opts);
...
extern set_t clique_unweighted_find_single(graph_t *g,int min_size,
					   int max_size,_Boolean maximal,
					   clique_options *opts);
extern int clique_unweighted_find_all(graph_t *g, int min_size, int max_size,
				      _Boolean maximal, clique_options *opts);

/* Time printing functions */
extern _Boolean clique_print_time(int level, int i, int n, int max,
				 double cputime, double realtime,
				 clique_options *opts);
extern _Boolean clique_print_time_always(int level, int i, int n, int max,
					double cputime, double realtime,
					clique_options *opts);
...
extern int find_clique(graph *g, int m, int n,
				     int min, int max, _Boolean maximal);
