                               ...
                               std::pair<int, int> &>
{
    ...
    read_graph6_edges(input_iterator &start) : _in(&start),
                                               eos(false),
                                               num_nodes(0),
                                               packet_size(5),
                                               column(1),
    ...
    void parse_num_nodes()
    {
        ...
        //Read the header
        if (*in == '>')
        { //We have the optional >>graph6<< header
...
void graph6_to_edges_console(Input graph6str)
{
    Input eos;
    typedef read_graph6_edges<Input> edge_reader;
    edge_reader end;
    while (graph6str != eos)
    {
        read_graph6_edges<Input> edges(graph6str);
        read_graph6_edges<Input> last;
        std::cout << edges.num_nodes << std::endl;
        while (edges!=last){
            std::cout<<edges.column<<" "<<edges.row<<std::endl;
            edges++;
        }
        if (*graph6str++ != '\n')
