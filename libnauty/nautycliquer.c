...
static _Boolean sub_unweighted_single(int *table, int size, int min_size,
				     graph_t *g);
static int sub_unweighted_all(int *table, int size, int min_size, int max_size,
			      _Boolean maximal, graph_t *g,
			      clique_options *opts);
static int sub_weighted_all(int *table, int size, int weight,
			    int current_weight, int prune_low, int prune_high,
			    int min_weight, int max_weight, _Boolean maximal,
			    graph_t *g, clique_options *opts);


static _Boolean store_clique(set_t clique, graph_t *g, clique_options *opts);
static _Boolean is_maximal(set_t clique, graph_t *g);
static _Boolean false_function(set_t clique,graph_t *g,clique_options *opts);
 ...
 */
_Boolean clique_print_time(int level, int i, int n, int max,
 ...
 * Returns always TRUE  (ie. never requests abort).
 */
_Boolean clique_print_time_always(int level, int i, int n, int max,
				 double cputime, double realtime,
				 clique_options *opts) {
	static float prev_time=100;
	static int prev_i=100;
 ...
 */
_Boolean graph_weighted(graph_t *g) {
	int i,w;
 ...
 */
_Boolean graph_test(graph_t *g,FILE *output) {
	int i,j;
	int edges=0;
	int asymm=0;
	int nonpos=0;
 ...
 */
_Boolean reorder_is_bijection(int *order,int n) {
	_Boolean *used;
	int i;

	used=calloc(n,sizeof(_Boolean));
	for (i=0; i<n; i++) {
 ...
 */
int *reorder_by_ident(graph_t *g,_Boolean weighted) {
 ...
 */
int *reorder_by_reverse(graph_t *g,_Boolean weighted) {
	int i;
	int *order;
 ...
 */
int *reorder_by_greedy_coloring(graph_t *g,_Boolean weighted) {
	if (weighted)
 ...
 */
int *reorder_by_unweighted_greedy_coloring(graph_t *g,_Boolean weighted) {
	int i,j,v;
	_Boolean *tmp_used;
	...
	tmp_used=calloc(g->n,sizeof(_Boolean));
	...
	while (v < g->n) {
		/* Reset tmp_used. */
		memset(tmp_used,0,g->n * sizeof(_Boolean));
 ...
 */
int *reorder_by_weighted_greedy_coloring(graph_t *g, _Boolean weighted) {
	int i,j,p=0;
	int cnt;
	...
	order=malloc(g->n * sizeof(int));
	used=calloc(g->n,sizeof(_Boolean));
 ...
 */
int *reorder_by_degree(graph_t *g, _Boolean weighted) {
	int i,j,v;
	int *degree;
	int *order;
	int maxdegree,maxvertex=0;
 ...
 */
int *reorder_by_random(graph_t *g, _Boolean weighted) {
	/* struct tms t; */
	int i,r;
	int *new;
	_Boolean *used;
...
int
find_clique(graph *g, int m, int n, int min, int max, _Boolean maximal)
...
int
find_indset(graph *g, int m, int n, int min, int max, _Boolean maximal)
