...
static _Boolean filterschreier(schreier*,int*,permnode**,_Boolean,int,int);
...
filterschreier(schreier *gp, int *p, permnode **ring,
               _Boolean ingroup, int maxlevel, int n)
 ...
 * Return TRUE iff some change is made. */
{
    ...
    int *orbits,*pwr;
    permnode **vec,*curr;
    _Boolean changed,lchanged,ident;
#if !MAXN
...
getorbitsmin(int *fix, int nfix, schreier *gp, permnode **ring,
         int **orbits, int *cell, int ncell, int n, _Boolean changed)
