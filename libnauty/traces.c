...
typedef struct TracesInfo {
    _Boolean autofound;
    _Boolean deg_one;
    _Boolean first_matching;
    _Boolean regular;
    _Boolean exitfromref;
    _Boolean identitygroup;
    _Boolean minimalinorbits;
    _Boolean thegraphisparse;
    _Boolean thegrouphaschanged;
    _Boolean thereisnextlevel;
    _Boolean useTempOrbits1;
    _Boolean useTempOrbits2;
} TracesInfo;

typedef struct TracesSpine {
    _Boolean thetracexists;
    Candidate *listend;
    Candidate *liststart;
    int ccend;
    int ccstart;
...
static _Boolean traces_degree_refine(sparsegraph*, Candidate*, int, Partition*,
...
static int  traces_refine(Candidate*, int, Partition*,
                          struct TracesVars*, struct TracesInfo*, int, _Boolean);
...
static void  refine_tr_refine(Candidate*, int, Partition*,
                             struct TracesVars*, struct TracesInfo*);
static int given_gens(sparsegraph*, permnode*, int*, _Boolean);
...
static _Boolean FixedBase(int*, struct TracesVars*, Candidate*, int, int);
...
static _Boolean TreeFyTwo(int, Candidate*, Candidate*, Partition*, int, struct TracesVars*, struct TracesInfo*);
static void ExperimentalStep(Partition*, Candidate*, struct TracesVars*, struct TracesInfo*, int, int);
static _Boolean TargetCell(Candidate*, Partition*, int, struct TracesVars*, int);
static _Boolean TargetCellFirstPath(Candidate*, Partition*, struct TracesVars*);
static int TargetCellExpPath(Candidate*, Partition*, struct TracesVars*);
static _Boolean TargetCellSmall(Candidate*, Partition*, int, struct TracesVars*, int);
static _Boolean TargetCellFirstPathSmall(Candidate*, Partition*, struct TracesVars*);
static int TargetCellExpPathSmall(Candidate*, Partition*, struct TracesVars*);
static _Boolean SelectNextLevel(int, struct TracesVars*, struct TracesInfo*);
...
static void RemoveFromLevel(int, int, int, _Boolean);
...
static _Boolean Prefix(Candidate*, Candidate*, int);
static _Boolean findperm(permnode*, int*, int);
...
static _Boolean lookup(searchtrie*);
...
static void MakeTree(int, int, sparsegraph*, int, struct TracesVars*, _Boolean);
...
static _Boolean isautom_sg_pair(graph*, int*, _Boolean, int, int, struct TracesVars*);
...
static _Boolean VerifyId(int *p, int n);
...
static _Boolean VerifyPart(Partition*, int, int);
static int VerifyPerm(int*, int,int);
static _Boolean VerifyCand(Candidate*, int, int);
...
DYNALLSTAT(_Boolean, Diff, Diff_sz);
DYNALLSTAT(_Boolean, Factorials, Factorials_sz);
...
static TLS_ATTR _Boolean Diff[MAXN];
static TLS_ATTR _Boolean Factorials[MAXN];
...
Traces(sparsegraph *g_arg, int *lab, int *ptn,
    ...
    if (tv->preprocessed) {
        memset(Diff,0,n*sizeof(_Boolean));
        for (i=0; i<n; i++) {
...
void Allocate_Traces_Structures(int n) {
    ...
    DYNALLOC1(_Boolean, Diff, Diff_sz, n, "Traces");
    DYNALLOC1(int, CStack, CStack_sz, n, "Traces");
    DYNALLOC1(_Boolean, Factorials, Factorials_sz, n, "Traces");
...
int CompStage0(Partition *CurrPart, Partition *NextPart, Candidate *CurrCand, Candidate *NextCand,
    ...
    _Boolean closeloop, firstsing, has_nexttcell;
...
int CompStage2(Partition *CurrPart, Partition *NextPart, Candidate *CurrCand, Candidate *NextCand,
    ...
    searchtrie *TreeNode, *TreeNode1, *TreeNode2;
    int *CuOrb,*AuxOrb;
    _Boolean has_nexttcell = FALSE;
    searchtrie *TrieNode;
    _Boolean schreierwrong;
...
_Boolean findperm(permnode *pn, int *p, int n) {
    permnode *rn;
...
_Boolean FixedBase(int *fix, struct TracesVars *tv, Candidate *Cand, int from, int to) {
    int i, k, nfix;
...
int given_gens(sparsegraph *g, permnode *gens, int *orbits, _Boolean digraph) {
    int i, m, n, norbs;
    permnode *pn;
...
_Boolean isautom_sg_pair(graph *g, int *p, _Boolean digraph, int m, int n, struct TracesVars *tv) {
    int *d, *e;
    size_t *v;
    int i, k, pi, di;
    size_t vi, vpi, j;
...
void MakeTree(int v1, int v2, sparsegraph *sg, int n, struct TracesVars* tv, _Boolean forceautom) {
    int ind, vtx1, vtx2, ngh1, ngh2, trind, deg0, deg1;
    size_t j1;
    int *sge1, *sge2;
    _Boolean build_autom;
...
_Boolean Prefix(Candidate *Cand1, Candidate *Cand2, int k) {
    int i;
...
void RemoveFromLevel(int from, int to, int strategy, _Boolean reinit) {
    int i;
...
_Boolean SelectNextLevel(int n, struct TracesVars *tv, struct TracesInfo *ti) {
    int i, j, val;
    Candidate *FirstCand;
    _Boolean orbitcell;
...
_Boolean TargetCell(Candidate *TargCand, Partition *Part, int n, struct TracesVars* tv, int Lv) {
...
_Boolean TargetCellFirstPath(Candidate *TargCand, Partition *Part, struct TracesVars* tv) {
...
_Boolean TreeFyTwo(int From, Candidate *Cand1, Candidate *Cand2, Partition *Part, int n,
                  struct TracesVars* tv, struct TracesInfo *ti) {
    int i, i1, i2, j1, j2, k;
...
_Boolean VerifyCand(Candidate *Cand, int n, int line) {
    int i, k;
...
_Boolean VerifyPart(Partition *Part, int start, int end) {
    int i,j;
...
_Boolean TargetCellSmall(Candidate *TargCand, Partition *Part, int n, struct TracesVars* tv, int Lv) {
...
_Boolean TargetCellFirstPathSmall(Candidate *TargCand, Partition *Part, struct TracesVars* tv) {
    int n, TCell, TCSize, TCell1, TCSize1;
    int Lv, i, Lev, vtx, vtx_d;
    int loopstart, loopend;
    _Boolean divided;
