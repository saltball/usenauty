...
main(int argc, char *argv[])
{
        ...
        FILE *infile,*outfile;
        _Boolean badargs,dolabel,quiet,Dswitch;
	_Boolean bswitch,tswitch,fswitch,Fswitch,zswitch;
	int mincycle;
	...
	_Boolean okdist[MAXN+1];
#else
	DYNALLSTAT(graph,h,h_sz);
	DYNALLSTAT(int,deg,deg_sz);
	DYNALLSTAT(_Boolean,okdist,okdist_sz);
	...
	while (TRUE)
	{
	    ...
	    DYNALLOC1(_Boolean,okdist,okdist_sz,n+1,"addedgeg");
