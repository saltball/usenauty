...
static void
print_counts(countnode *root, _Boolean printcounts)
...
add_one(countnode **to_root, char *fmt, integer pmod, int nval,
        number *val, int *valtype, int which, _Boolean numcompare)
...
*/
{
    ...
    int digit;
    _Boolean doass,neg,oflow,badgmp;
    integer ival;
    double dval,digval,comval;
    char ends,*saves;
    static _Boolean gmp_warning = FALSE;
    integer *ilist;
    size_t ilist_sz;
    int nilist;
#if GMP
...
static void
read_formats(char *filename, int *numformatsp, _Boolean mustexist)
...
main(int argc, char *argv[])
{
    ...
    _Boolean havefinal,nowarn,noWarn,listformats,readfiles;
    integer seq;
    int order[MAXFORMATS];
    glob_t globlk,globlk_stdin,*pglob;
    char *glob_stdin_v[2];
    _Boolean printcounts;
