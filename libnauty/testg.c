...
\n\
  Miscellaneous switches:\n\
     ...
     -1          Write output as lines of numbers separated by spaces,\n\
                 with 0/1 for _Boolean and both endpoints of ranges given\n\
...
{
    char symbol;
    _Boolean digraphok;   /* Can be computed for digraphs */
    _Boolean rangeonly;   /* Display a range */
...
static _Boolean docount,dofilter;
static _Boolean rangemarkerseen = FALSE;  /* --: seen */
...
#define INSERT_ARGS , _Boolean isvalue, SPLAYNODE *new_val, size_t new_val_sz
...
static _Boolean lswitch,oneswitch,twoswitch;
...
static void
groupstats(graph *g, _Boolean digraph, int m, int n, group_node *sz,
...
static void
compute(graph *g, int m, int n, int code, _Boolean digraph)
...
static _Boolean
selected(graph *g, int m, int n, _Boolean digraph)
...
/* Extract key symbols from -- string */
{
	...
	{
	    fprintf(stderr,
                    ">W ignoring unsplit _Boolean property %c\n",s[i]);
	    continue;
	}
...
main(int argc, char *argv[])
{
    ...
    _Boolean badargs,lastwritten,digraph;
    long pval1,pval2,maxin;
    _Boolean fswitch,pswitch,Vswitch,vswitch,Xswitch,qswitch;
    nauty_counter cmask;
    long arglo,arghi;
    _Boolean havecon,neg,doflush,isselected;
    double t;
